                  ...
                  {link.hasDropdown ? (
                    <div
                      ...
                      onMouseEnter={() => {
                        ...
                        else if (link.dropdownType === 'trades') setIsTradesOpen(true);
                      }}
                      onMouseLeave={() => {
                        if (link.dropdownType === 'services') setIsServicesOpen(false);
                        else if (link.dropdownType === 'trades') setIsTradesOpen(false);
                      }}
                    >
                      <button
                        className={`flex items-center gap-1 px-4 py-2 text-sm font-medium transition-colors rounded-lg ${(link.dropdownType === 'services' && location.pathname.startsWith('/services')) ||
                      ...
                      >
                        {link.name}
                        <HiChevronDown className={`w-4 h-4 transition-transform ${(link.dropdownType === 'services' && isServicesOpen) ||
                        ...
                        {((link.dropdownType === 'services' && isServicesOpen) ||
                          (link.dropdownType === 'trades' && isTradesOpen)) && (
                            <motion.div
                              ...
                              className={`absolute top-full left-1/2 -translate-x-1/2 mt-4 bg-card rounded-2xl shadow-strong border border-border/50 overflow-hidden ${link.dropdownType === 'services' ? 'w-[500px]' : 'w-[600px]'}`}
                            >
                              <div className="p-2">
                                <div className={`grid gap-2 ${link.dropdownType === 'services' ? 'grid-cols-1' : 'grid-cols-2'
                                  }`}>
                    ...
                    <div key={link.name}>
                      {link.hasDropdown ? (
                        <div>
                          <button
                            onClick={() => {
                              if (link.dropdownType === 'services') setIsServicesOpen(!isServicesOpen);
                              else if (link.dropdownType === 'trades') setIsTradesOpen(!isTradesOpen);
                            }}
